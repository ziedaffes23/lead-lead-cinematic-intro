...
export function CinematicIntro({ onIntroComplete, reducedMotion = false }: CinematicIntroProps) {
  ...
  useEffect(() => {
    ...
    const startMusic = () => {
      if (started) return;
      started = true;
      void audio.play().catch(() => { started = false; });
    };
    const retryOnInteraction = () => startMusic();
    startMusic();
  ...
  return (
    <main className={`cinematic-shell ${reducedMotion ? "is-static" : ""}`} aria-label="Lead & Lead opening cinematic">
      ...
      <section ref={revealRef} className={`cinematic-reveal ${revealActive ? "is-active" : ""}`} aria-hidden={!revealActive}>
        ...
        <div className="presentation-lockup"><div className="thyna-seal"><img className="official-logo thyna-intro-mark" src={CINEMATIC_ASSETS.logo} alt="LC Thyna" /></div><div className="presentation-line"><i /><span>LC THYNA</span><i /></div><p>PROUDLY PRESENTS</p><h1>LEAD <em>&</em> LEAD <small>2K26</small></h1><div className="reveal-subtitle"><span>THE LEADERSHIP GATHERING</span><i /><strong>10 SEPTEMBER 2026</strong></div></div>
