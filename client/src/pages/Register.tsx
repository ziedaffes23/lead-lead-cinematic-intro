...
export const SHEETS_WEB_APP_URL = import.meta.env.VITE_SHEETS_WEB_APP_URL ?? "";
...
type Nationality = "" | "Tounsi" | "Other";
type Position = "" | "MMB" | "EB";
type FormState = { firstName: string; lastName: string; cin: string; lc: string; phoneCountry: string; phone: string; email: string; nationality: Nationality; nationalityDetail: string; position: Position; singleRoom: boolean; department: string; allergies: string; note: string; };
...
const initial: FormState = { firstName: "", lastName: "", cin: "", lc: "LC Thyna", phoneCountry: "+216", phone: "", email: "", nationality: "", nationalityDetail: "", position: "", singleRoom: false, department: "", allergies: "", note: "" };
const informationFields: Array<keyof FormState> = ["firstName", "lastName", "cin", "lc", "phoneCountry", "phone", "email"];
const participationFields: Array<keyof FormState> = ["nationality", "nationalityDetail", "position", "department", "allergies", "note"];

function contribution(form: Pick<FormState, "nationality" | "position" | "singleRoom">) {
  return getContribution(form.nationality, form.position, form.singleRoom);
}
...
export default function Register() {
  ...
  const [status, setStatus] = useState<"idle" | "sending" | "upload" | "error" | "endpoint">("idle");
  ...
  const fee = useMemo(() => contribution(form), [form.nationality, form.position, form.singleRoom]);
  ...
  const validate = (fields: Array<keyof FormState>) => {
    ...
    if (fields.includes("nationalityDetail") && form.nationality === "Other" && !form.nationalityDetail.trim()) next.nationalityDetail = "Please specify your nationality.";
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!SHEETS_WEB_APP_URL) { setStatus("endpoint"); return; }
  ...
  return (
    <main className="registration-site chase-route chase-register cinematic-world-root">
      ...
      <div className="register-layout">
        <aside className="dossier-intro"><p className="eyebrow">CHAPTER VI / DELEGATE DOSSIER</p><h1>{stage === "receipt" ? "The record is sealed." : "Answer the call."}</h1><dl><div><dt>STARTS</dt><dd>10 September 2026</dd></div><div><dt>DURATION</dt><dd>3 days</dd></div><div><dt>HOST</dt><dd>LC Thyna</dd></div><div><dt>VENUE</dt><dd>Amir Palace</dd></div></dl></aside>
