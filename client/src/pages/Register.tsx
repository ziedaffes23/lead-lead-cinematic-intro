...
type FormState = { firstName: string; lastName: string; gender: Gender; cin: string; lc: string; phoneCountry: string; phone: string; email: string; nationality: Nationality; track: Track; position: Position; singleRoom: boolean; department: string; allergies: string; note: string; indemnitySignature: string; indemnityAccepted: boolean; };
...
const initial: FormState = { firstName: "", lastName: "", gender: "", cin: "", lc: "", phoneCountry: "+216", phone: "", email: "", nationality: "Tunisian", track: "", position: "", singleRoom: false, department: "", allergies: "", note: "", indemnitySignature: "", indemnityAccepted: false };
const informationFields: Array<keyof FormState> = ["firstName", "lastName", "gender", "cin", "lc", "phoneCountry", "phone", "email"];
...
export default function Register() {
  ...
  const validate = (fields: Array<keyof FormState>) => {
    ...
    if (fields.includes("gender") && !form.gender) next.gender = "Select your gender.";
  ...
  return (
    <main className="registration-site chase-route chase-register cinematic-world-root">
      ...
      <div className="register-layout">
        <aside className="dossier-intro"><p className="eyebrow">CHAPTER VI / DELEGATE DOSSIER</p><h1>{stage === "receipt" ? "The record is sealed." : "Answer the call."}</h1><dl><div><dt>STARTS</dt><dd>10 September 2026</dd></div><div><dt>DURATION</dt><dd>{form.track ? `${form.track} · ${form.track === "MMB" ? "3" : "4"} days` : "MMB · 3 days / EB · 4 days"}</dd></div><div><dt>HOST</dt><dd>LC Thyna</dd></div><div><dt>VENUE</dt><dd>Hotel monastir centre</dd></div></dl></aside>
