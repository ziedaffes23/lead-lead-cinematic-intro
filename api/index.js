...
var DEFAULT_SHEETS_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbw4AbEAqAdrUHhXaOOq2OoNJFKKhjpKQA8_SkWmhHNlYLh0cayQnIEUpTSXCO9S3H5kfw/exec";
...
function getSheetsWebAppUrl() {
  ...
  return configured && !LEGACY_SHEETS_WEB_APP_URLS.has(configured) ? configured : DEFAULT_SHEETS_WEB_APP_URL;
}
...
// server/registrationSubmission.ts
var publicUrl = z3.string().url().refine(
  (value) => value.startsWith("https://"),
  "Document URLs must use HTTPS."
);
var documentReference = z3.union([
  publicUrl,
  z3.string().regex(/^data:[^;,]+;base64,[A-Za-z0-9+/]+={0,2}$/, "Invalid document data.")
]);
var registrationSubmissionInput = z3.object({
  ...
  phone: z3.string().trim().regex(/^\d{8}$/, "Phone number must contain exactly 8 digits."),
  email: z3.string().trim().email(),
  nationality: z3.literal("Tunisian"),
  track: z3.enum(["MMB", "EB"]),
  position: z3.enum(["Manager", "Team Leader", "LCVP", "LCP"]),
...
async function submitRegistrationToSheets(input) {
  ...
  try {
    ...
    const finalResponse = redirectStatuses.has(response.status) ? await (async () => {
